...
class TestBgm:
    def test_play_music_if_ES_is_running(self):
        ...
        app = (
            scenario_maker.theFollowingProcessesAreRunning("emulationstatio")
    ...
    def test_fade_up_music_if_Emulator_is_not_running_and_ES_is_running_and_music_is_paused(
        self,
    ):
        scenario_maker = self._ScenarioMaker()
        app = (
    ...
    def test_do_nothing_if_ES_is_not_running_but_emulator_is_running(self):
        scenario_maker = self._ScenarioMaker()
        app = (
            scenario_maker.theFollowingSongsArePresent(
    ...
    class _ScenarioMaker:
        def __init__(self):
            ...
            self.processService.process_is_running = MagicMock(
        ...
        def theFollowingProcessesAreRunning(self, *processes):
            self.processService.process_is_running = MagicMock(
                side_effect=lambda x: x in processes
            )
            return self
        ...
        def build(self):
            ...
            return MusicStateMachine(
                self.processService, self.musicPlayer, self.config, self.forced_state
            )
